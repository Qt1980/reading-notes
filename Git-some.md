...
Using GIT we can make changes to files by cloning our files stored on the repository located on GitHub using the ```git clone (website address)``` command. The address to each repository is unique. This is the checkout process that clones from the Repository to our local hard disk by using the ```git clone Https:...```.  
Now we can begin to make changes to files found in the repository store on our local machine through a text editor. I'm using Microsoft's VS Code editor on my machine but you use whatever you feel comfortable with.